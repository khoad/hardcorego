...
func (s sysadmin) administrate(system string) {
	println("Name:", s.name, "is administering system:", system)
}
...
func (p programmer) develop(system string) {
	println("Name:", p.name, "is developing system:", system)
}
...
var tasks = []struct {
	...
	//{system: "project-alpha", needsDev: true},
}
...
func main() {
	...
	admins.pushAdmin(sysadmin{"Bob"})

	// Push two new programmers onto devs.
	devs.pushDev(programmer{"Jack"})
	devs.pushDev(programmer{"John"})
	...
	for _, task := range tasks {
		...
		if task.needsDev {
			dev := devs.popDev()
			fmt.Printf("Dev value:%+v of type:%T \n", dev, dev)
		...
		if task.needsAdmin {
			...
			fmt.Printf("Admin value:%+v of type:%T\n", admin, admin)
